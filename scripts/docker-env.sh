...
get-build-args() {
    ...
    # Grab the servicw args
    YABONZA_SERVICE=$2
    SERVICE_ARGS="";
    if [ "$YABONZA_SERVICE" == "yabonza/yabonza-api" ]; then
        SERVICE_ARGS="$(get-api-args)";
    elif [ "$YABONZA_SERVICE" == "yabonza/yabonza-fund" ]; then
        SERVICE_ARGS="$(get-fund-args)";
    elif [ "$YABONZA_SERVICE" == "yabonza/yabonza-web" ]; then
